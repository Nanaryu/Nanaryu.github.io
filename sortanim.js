...
async function shuffle(array)
{
	...
	while (currentIndex > 0 && isRunning)
	{
		...
		if (Math.random() < 100 / array.length) // it should work
		{
			updateArray();
			await sleep(0);
		}
...
async function merge(data, start = 0, end = null, firstCall = false)
{
	if(!isRunning)
	{
		return;
	...
	await merge(data, start, mid);
	await merge(data, mid + 1, end);
...
async function quicksort(arr, low, high, firstCall=false) {
	if (low >= high) return;
